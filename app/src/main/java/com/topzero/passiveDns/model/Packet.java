...
public class Packet {
    ...
    public static class IP4Header {
        public byte version;  // IP版本号
        public byte IHL;  // 报头长度
        public int headerLength;  // 报头长度int值
        public short typeOfService;  //服务类型
        public int totalLength;  // 总长度int值
        public int identificationAndFlagsAndFragmentOffset;  // 标识符、标志和片偏移
        public short TTL;  // 生存时间
        private short protocolNum;  // 协议号
        public TransportProtocol protocol; //协议
        public int headerChecksum; // 校验和
        public InetAddress sourceAddress;  // 源IP地址
        public InetAddress destinationAddress;  // 目的IP地址
        public int optionsAndPadding;  // 可选字段和填充

        // 定义数据结构 （TCP协议号6，UDP协议号17）
    ...
    public static class TCPHeader {
        ...
        public long sequenceNumber;  // 请求号
        public long acknowledgementNumber;  // 应答号

        public byte dataOffsetAndReserved;  // 数据偏移和保留
        public int headerLength;  // tcp头部长度
        public byte flags;  // 标志位
        public int window;  // 窗口

        public int checksum;  // 校验和
        public int urgentPointer;  // 紧急指针

        public byte[] optionsAndPadding;  // 可选字段和填充
